...
void measure_mixed(uint8_t n_b, uint8_t n_a, uint64_t stride, uint64_t training_data_length, uint8_t covert_channel)
{
    ...
    {
        transfered = transfer_mixed(asm_encode_load_dynamic, decode_flush_reload_dynamic, secret, 0, threshold, n_b, n_a, stride + 64, training_data_length, covert_channel);
    }
    else
    {
